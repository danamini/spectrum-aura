...
\[
T_{\text{window}} = \frac{N}{f_s}
\]

Example: \(N=2048\), \(f_s=48000\) → \(T_{\text{window}} \approx 42.7\,\text{ms}\).
...
\[
\text{bass} = \frac{1}{|\mathcal{B}|}\sum_{k \in \mathcal{B}} v_k
\]
...
\[
\Delta_b = b_t - b_{t-1},\quad \text{etc.}
\]
...
\[
\bar{b} = \frac{1}{W}\sum_{i=0}^{W-1} b_{t-i}
\]
...
\[
\Delta t_{\min} = \begin{cases} 160\,\text{ms} & \bar{b} > 0.35 \\ 180\,\text{ms} & \text{otherwise} \end{cases}
\]
...
\[
CV = \frac{\sigma_{\Delta t}}{\mu_{\Delta t}},\qquad
...
\[
\phi_{\text{det}}(t) = \frac{(t - t_{\text{anchor}}) \bmod T_{\text{beat}}}{T_{\text{beat}}},\quad
T_{\text{beat}} = \frac{60000}{\text{BPM}}
\]
